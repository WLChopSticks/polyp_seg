...
def updateMask(train_image_dir, train_csv_path, mask_dir, model, index):
    ...
    for j in range(len(dataframe)):
        ...
        update_mask_with_id_maskdir(j, image_path, mask_path, test_img_aug,new_mask_path, model)
...
def update_mask_with_id_maskdir(j,image_path,mask_path, transform,new_mask_path, model):
    ...
    #网络输出与超像素比对
    t = Thread(target=compare_predict_spixel, args=(j, img, mask_origin_size, gt,new_mask_path))
    t.start()
    #compare_predict_spixel(j, img, mask_origin_size, gt)
...
def compare_predict_spixel(j, img, mask_origin_size, gt, new_mask_path):
    image = np.array(img)
